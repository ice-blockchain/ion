...
\begin{abstract}
  The aim of this text is to provide a brief description of Fift, a new programming language specifically designed for creating and managing ION Blockchain smart contracts, and its features used for interaction with the ION Virtual Machine~\cite{TVM} and the ION Blockchain~\cite{TBC}.
\end{abstract}

\section*{Introduction}
\markbothsame{Introduction}

This document provides a brief description of Fift, a stack-based general-purpose programming language optimized for creating, debugging, and managing ION Blockchain smart contracts.

Fift has been specifically designed to interact with the ION Virtual Machine (ION VM or TVM)~\cite{TVM} and the ION Blockchain~\cite{TBC}. In particular, it offers native support for 257-bit integer arithmetic and TVM cell manipulation shared with TVM, as well as an interface to the Ed25519-based cryptography employed by the ION Blockchain. A macro assembler for TVM code, useful for writing new smart contracts, is also included in the Fift distribution.
...
We have discussed the basic Fift primitives not related to TVM or the ION Blockchain so far. Now we will turn to ION-specific words, used to manipulate {\em Cell\/}s.
...
\item {\tt boc+>B} ($c$ $x$ -- $B$), creates and serializes a ``standard'' bag of cells, containing one root {\em Cell\/}~$c$ along with all its descendants. An {\em Integer\/} parameter $0\leq x\leq 31$ is used to pass flags indicating the additional options for bag-of-cells serialization, with individual bits having the following effect:
  ...
  \end{itemize}
Typical values of $x$ are $x=0$ or $x=2$ for very small bags of cells (e.g., ION Blockchain external messages) and $x=31$ for large bags of cells (e.g., ION Blockchain blocks).
...
\mysection{ION-specific operations}
This chapter describes the ION-specific Fift words, with the exception of the words used for {\em Cell\/} manipulation, already discussed in the previous chapter.
...
Two special words can be used to parse ION smart-contract addresses in human-readable (base64 or base64url) forms:
\begin{itemize}
...
rot . swap x. . cr
\end{verbatim}
outputs ``{\tt -1 538fa7\dots 0f7d 0}'', meaning that the specified address is in workchain $-1$ (the masterchain of the ION Blockchain), and that the 256-bit address inside workchain $-1$ is 0x538\dots f7d.
...
\item {\tt runvmctx} (\dots $s$ $c$ $t$ -- \dots $x$ $c'$), a variant of {\tt runvm} that also initializes {\tt c7} (the ``context'') with {\em Tuple\/}~$t$. In this way the execution of a TVM smart contract inside ION Blockchain can be completely emulated, if the correct context is loaded into {\tt c7} (cf.~\cite[4.4.10]{TBC}).
...
Larger TVM programs, such as ION Blockchain smart contracts, typically consist of several mutually recursive subroutines, with one or several of them selected as top-level subroutines (called {\tt main()} or {\tt recv\_internal()} for smart contracts). The execution starts from one of the top-level subroutines, which is free to call any of the other defined subroutines, which in turn can call whatever other subroutines they need.
...
\item Other predefined subroutine selectors such as {\tt recv\_internal} (equal to $0$) or {\tt recv\_external} (equal to $-1$), useful for implementing ION Blockchain smart contracts (cf.~\cite[4.4]{TBC}), can be declared by means of {\tt constant} (e.g., {\tt -1 constant recv\_external}).
...
\item {\tt single} ($x$ -- $t$), creates new singleion $t=(x)$, i.e., a one-element {\em Tuple}. Equivalent to {\tt 1 tuple}.
...
\item {\tt unsingle} ($t$ -- $x$), unpacks a singleion $t=(x)$. Equivalent to {\tt 1 untuple}.
