...
int main() {
  ...
  scheduler.run_in_context([&] {
    ...
    for (td::uint32 i = 0; i < total_nodes; i++) {
      ...
      if (i == 0) {
        auto obj = ion::create_tl_object<ion::ton_api::dht_node>(pub1.tl(), addr0.tl(), -1, td::BufferSlice());
        auto d = pk1.create_decryptor().move_as_ok();
        obj->signature_ = d->sign(serialize_tl_object(obj, true)).move_as_ok();

        std::vector<ion::tl_object_ptr<ion::ton_api::dht_node>> vec;
        vec.push_back(std::move(obj));
        auto nodes = ion::create_tl_object<ion::ton_api::dht_nodes>(std::move(vec));
        auto conf = ion::create_tl_object<ion::ton_api::dht_config_global>(std::move(nodes), 6, 3);
...

  {
    ...
    auto pk_anybody = ion::PrivateKey{ion::privkeys::Unenc{x.clone()}};
    auto pub_anybody = pk_anybody.compute_public_key();
    auto short_id_anybody = pub_anybody.compute_short_id();

    ion::dht::DhtKey dht_key{short_id_anybody, "test", 0};
    auto dht_update_rule = ion::dht::DhtUpdateRuleAnybody::create().move_as_ok();
    ion::dht::DhtKeyDescription dht_key_description{std::move(dht_key), pub_anybody, std::move(dht_update_rule),
                                                    td::BufferSlice()};
    dht_key_description.check().ensure();
...

  {
    ...
    auto overlay_pub = ion::PublicKey{ion::pubkeys::Overlay{overlay_name.clone()}};
    auto overlay_short_id = overlay_pub.compute_short_id();

    ion::dht::DhtKey dht_key{overlay_short_id, "test", 0};
    auto dht_update_rule = ion::dht::DhtUpdateRuleOverlayNodes::create().move_as_ok();
    ion::dht::DhtKeyDescription dht_key_description{std::move(dht_key), overlay_pub, std::move(dht_update_rule),
                                                    td::BufferSlice()};
    dht_key_description.check().ensure();
    ...
    auto obj = ion::create_tl_object<ion::ton_api::overlay_nodes>();
    dht_value =
    ...
    for (td::uint32 i = 0; i < 100; i++) {
      ...
      auto to_sign = ion::create_serialize_tl_object<ion::ton_api::overlay_node_toSign>(
          ion::adnl::AdnlNodeIdShort{pub.compute_short_id()}.tl(), overlay_short_id.tl(), date);
      auto n = ion::create_tl_object<ion::ton_api::overlay_node>(
    ...
    //overlay.node id:PublicKey overlay:int256 version:int signature:bytes = overlay.Node;
    auto to_sign = ion::create_serialize_tl_object<ion::ton_api::overlay_node_toSign>(
        ion::adnl::AdnlNodeIdShort{pub.compute_short_id()}.tl(), overlay_short_id.tl() ^ td::Bits256::ones(), date);
    auto n = ion::create_tl_object<ion::ton_api::overlay_node>(
    ...
    obj->nodes_.clear();
    to_sign = ion::create_serialize_tl_object<ion::ton_api::overlay_node_toSign>(
        ion::adnl::AdnlNodeIdShort{pub.compute_short_id()}.tl(), overlay_short_id.tl(), date);
    n = ion::create_tl_object<ion::ton_api::overlay_node>(
        pub.tl(), overlay_short_id.tl(), date,
        pk.create_decryptor().move_as_ok()->sign(to_sign.as_slice()).move_as_ok());
    obj->nodes_.push_back(std::move(n));
    dht_value =
    ...
    n = ion::create_tl_object<ion::ton_api::overlay_node>(pub.tl(), overlay_short_id.tl(), date, td::BufferSlice{64});
    obj->nodes_.push_back(std::move(n));
    dht_value =
        ion::dht::DhtValue{dht_key_description.clone(), ion::serialize_tl_object(obj, true), ttl, td::BufferSlice()};
    dht_value.check().ensure_error();
    ...
    CHECK(ion::fetch_tl_object<ion::ton_api::overlay_nodes>(dht_value.value().as_slice(), true)
              .move_as_ok()
              ->nodes_.size() == 1);
