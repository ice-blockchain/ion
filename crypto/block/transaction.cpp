...
td::RefInt256 StoragePrices::compute_storage_fees(ion::UnixTime now, const std::vector<block::StoragePrices>& pricing,
                                                  const StorageUsed& storage_used, ion::UnixTime last_paid,
 ...
 * See https://t.me/iondev_news/129
 ...
 */
static td::optional<td::uint64> override_gas_limit(const ComputePhaseConfig& cfg, ion::UnixTime now,
  ...
  static std::map<std::pair<ion::WorkchainId, ion::StdSmcAddress>, OverridenGasLimit> accounts = []() {
    auto parse_addr = [](const char* s) -> std::pair<ion::WorkchainId, ion::StdSmcAddress> {
      auto r_addr = StdAddress::parse(td::Slice(s));
      r_addr.ensure();
      return {r_addr.ok().workchain, r_addr.ok().addr};
    };
    std::map<std::pair<ion::WorkchainId, ion::StdSmcAddress>, OverridenGasLimit> accounts;
    ...
    // Increase limit for multiple accounts (https://t.me/iondev_news/129) until 2025-03-01 00:00:00 UTC
