...
\begin{abstract}
  ...
  Telegram Open Network (ION) and related blockchain, peer-to-peer,
  ...
  the unique and defining features of the ION platform that are
...
The {\em Telegram Open Network (ION)} is a fast, secure and scalable
...
think about ION as a huge distributed supercomputer, or rather a huge
...
\clearpage
\mysection{Brief Description of ION Components}\label{sect:ion.components}
...
For simplicity, we speak here about {\em the\/} ION Blockchain, even
...
\mysubsection{ION Blockchain as a Collection of 2-Blockchains}

The ION Blockchain is actually a {\em collection\/} of blockchains
...
The ION approach to sharding is ``bottom-up'', explained as follows.
...
We call this perspective the {\em Infinite Sharding Paradigm}. It explains many of the design decisions for the ION Blockchain.
...
paramount importance to a scalable system such as the ION
Blockchain. In fact, a novel feature of the ION Blockchain, called
...
shardchains.) A ION Blockchain contains exactly one
...
An important feature of the ION Blockchain is that it implements {\em
...
basic workchain, is the one used to work with {\em ION smart
  contracts\/} and transfer {\em ION coins}, also known as {\em
...
cf.~\ptref{sect:validators}.)  The ION Blockchain uses a
...
deposited {\em stakes\/} (large amounts of ION coins) by a special
...
messaging possible. In particular, each message may have some {\em
  value} attached, in the form of a certain amount of Grams (ION
...
\nxsubpoint\label{sp:ionvm} \embt(ION Virtual Machine.)  The {\em ION
  Virtual Machine}, also abbreviated as {\em ION VM\/} or {\em TVM\/},
...
important feature of the ION Block\-chain is that many of its
...
\nxsubpoint \embt(Relevance for the ION Blockchain.)  Recall that the
            {\em ION Block\-chain\/} is not a true blockchain, but a
...
which will be used in the formal specification of the ION Blockchain
...
(Type Language) will be used in the formal specifications of ION
...
\nxsubpoint\label{sp:hash.change} \embt(Hash used for the ION
Blockchain.)  We are using the 256-bit $\Sha$ hash for the ION
...
this global state. Here we describe the global state used by ION
blockchains.

\nxsubpoint \embt(Account IDs.)  The basic account IDs used by ION
...
component of the ION blockchain state is a {\em hashmap}. In some
...
\nxsubpoint \embt(Example: ION account balances.)  An important
example is given by ION account balances. It is a hashmap
\begin{equation}
  \vr{balance}:\Account\to\uint_{128}
\end{equation}
mapping $\Account=\st2^{256}$ into a Gram (ION coin) balance of type
...
such as ION.)  Notice that a node normally cannot be a full node for
all shardchains existing in the ION environment. It usually is a full
...
\nxsubpoint\label{sp:pec.tvm} \embt(Peculiarities of ION VM.)  The ION
...
In this way, the persistent data storage cells of a ION smart contract
...
for values of arbitrary algebraic types.)  Because the ION VM
...
\nxsubpoint\label{sp:tvm.data.shard} \embt(Support for sharding in ION
VM data structures.)  We have just outlined how the ION VM, without
...
level of ION VM. To this end, a special version of the hashmap type
...
feature of the ION Blockchain is the payment exacted from smart
...
An important component of the ION Blockchain is the {\em messaging
...
cryptocurrencies inside the ION Block\-chain pegged to Bitcoins,
...
servers, to implement the necessary interaction between the ION
...
receiving workchain and/or smart contract. For blockchains using ION
...
is obtained simply by recursively replacing all references in a ION VM
...
are not necessarily using the ION VM.
...
operation in ION VM), when some smart contract is invoked to process
...
shardchain, and is explicitly registered as such. Essentially, {\em
  all\/} ION Blockchain transactions consist in the delivery of one
...
multi-blockchain project such as ION to be able to forward and deliver
...
delivered more than once) and {\em quickly}. The ION Blockchain
...
Now we are ready to describe the global state of a ION blockchain, or
...
light version of the ION Blockchain client), enabling the receiver to
...
need to store or transfer, either in the ION Block\-chain or in the
network, is representable as a ``bag of cells''. This is an important
part of the ION Blockchain design philosophy. Once the ``bag of
...
services; cf.~\ptref{sp:ion.smart.pc.supp} for an example involving
payment channels.

\nxsubpoint \embt(Block ``headers'' for ION blockchains.)  Usually, a
...
In the ``bag of cells'' approach used by the blocks of ION
...
The ION Blockchain ultimately consists of shardchain and masterchain
...
become one, provided it can deposit a sufficiently large stake (in ION
...
for keeping the ION Blockchain working. This income is distributed
...
\nxsubpoint \embt(Validators instead of miners.)  Recall that the ION
...
resources to store and process ION Blockchain requests and data. In
...
mine new ION coins on a home computer, let alone a smartphone.
...
In order to become a validator, a node must transfer some ION coins
...
important consequence of the Proof-of-Stake approach used by the ION
Blockchain is that a light node (running light client software) for
the ION Blockchain does not need to download the ``headers'' of all
...
One of the most characteristic and unique features of the ION
...
We will conclude our brief discussion of the ION Blockchain by
...
explicitly exploited. Then it is a mixed {\em
  heterogeneous-homogeneous system}. To our knowledge, the ION
...
ION Blockchain uses dynamic sharding (cf.~\ptref{sect:split.merge}).
...
the ION Blockchain on our map. (In fact, we are indebted for our
...
(the slow version of) ION's as described in~\ptref{sp:hypercube}.
...
as their base, such as EOS, PolkaDot or ION. If a project needs
``true'' scalability (based on sharding), it would better use ION; if
...
\nxsubpoint \embt(The ION Blockchain.)  The ION (Telegram Open
...
components of the ION Project (cf.~\ptref{sect:ion.components}), its
...
The cornersione in building the ION networking protocols is the {\em
  (ION) Abstract (Datagram) Network Layer}. It enables all nodes to
...
perspective of almost all ION Networking components, the only thing
...
datagram carrying an embedded ION ADNL datagram will be equal to the
...
components of the ION Project do not need such a stream protocol.
...
\mysubsection{ION DHT: Kademlia-like Distributed Hash
  Table}\label{sect:kademlia}

The {\em ION Distributed Hash Table (DHT)\/} plays a crucial role in
the networking part of the ION Project, being used to locate other
...
a special key in the ION DHT. Another important application of the ION
...
entry point (e.g., IP address and UDP port) in the ION DHT; then all
nodes wishing to send datagrams to that node will obtain this contact
information from the DHT first.

The ION DHT is a member of the family of {\em Kademlia-like distributed
  hash tables\/}~\cite{Kademlia}.

\nxsubpoint \embt(Keys of the ION DHT.)  The {\em keys\/} of the ION
...
abstract addresses of the ION Network nodes (cf.~\ptref{sp:abs.addr})
can also be used as keys of the ION DHT, because they are also
...
identities.)  The key-value mapping of the ION DHT is kept on the {\em
  nodes\/} of the DHT---essentially, all members of the ION
Network. To this end, any node of the ION Network (perhaps with the
...
which identifies it as a member of the ION DHT. This {\em
...
\nxsubpoint \embt(Kademlia-like DHTs and the ION DHT.)  We say that a
...
The ION DHT is a Kademlia-like DHT, according to this definition. It
...
a Kademlia-like DHT usually maintains a {\em Kademlia routing
  table}. In the case of ION DHT, it consists of $n=256$ buckets,
...
A special feature of the ION DHT is that it tries to select nodes with
...
\item $\Store(key,value)$ -- Asks the node to keep $value$ as a value
  for key $key$. For ION DHT, the $\Store$ queries are slightly more
...
ION DHT, the value in this field is increased, and a new attempt is
...
services, located in the ION Network and available through the
(higher-level protocols built upon the) ION ADNL described
...
However, publishing the service's abstract address in the ION
...
An alternative is to publish a public key into the ION Blockchain, and
...
approaches exploited by ION Services.

\nxsubpoint \embt(Locating owners of ION blockchain accounts.)  In
most cases, owners of ION blockchain accounts would not like to be
...
however, the owner of a ION blockchain account may want to publish
one or several abstract addresses where she could be contacted.

A typical case is that of a node in the ION Payments ``lightning
network'' (cf.~\ptref{sect:lightning}), the platform for instant
cryptocurrency transfers. A public ION Payments node may want not only
...
controls the account in the ION Blockchain to sign and publish updates
in the ION DHT about the abstract addresses associated with that
...
some owners of the ION Blockchain accounts becomes available.

\nxsubpoint\label{sp:loc.abs.addr} \embt(Locating abstract addresses.)
Notice that the ION DHT, while being implemented over ION ADNL, is
itself used by the ION ADNL for several purposes.
...
the ION ADNL should be able to send datagrams to arbitrary 256-bit
...
In a multi-blockchain system like the ION Blockchain, even full nodes
...
(sub)network must be built inside the ION Network, on top of the ADNL
...
\nxsubpoint\label{sp:ion.overlays} \embt(Overlay networks in ION.)
Overlay networks in ION are built upon the ADNL protocol discussed
...
In contrast to ADNL, the ION overlay networks usually do not support
...
(i.e., from a node to one of its neighbors). In this way, a ION
overlay network is a (usually not full) subgraph inside the (full)
graph of the ADNL network.

Links to neighbors in ION overlay networks can be implemented using
...
name of the overlay network, a ION Blockchain shard identifier if this
...
ION DHT. In other cases (e.g., for private overlay networks), one must
...
in~\ptref{sp:distr.torr.tr}, also implemented with the aid of the ION
DHT.
...
in~\ptref{sp:ion.overlays}). After that, special messages are sent to
...
\nxsubpoint\label{sp:ov.opt.low.lat} \embt(ION overlay networks are
optimized for lower latency.)  ION overlay networks optimize the
...
for ION overlay networks, used, for example, to propagate block
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\clearpage
\mysection{ION Services and Applications}\label{sect:services}

We have discussed the ION Blockchain and ION Networking technologies
...
of the services that will be provided by the ION Project itself,
either from the very beginning or at a later time.

\mysubsection{ION Service Implementation Strategies}%
\label{sect:ion.service.impl}
...
the ION ecosystem. First of all, a simple classification is in order:
...
services. For example, ION Storage is a service, because it is
...
Telegram messenger, if made available through the ION Network (i.e.,
implemented as a ``ion-service''; cf.~\ptref{sp:telegram.ion.serv}),
...
for the ION ecosystem needs to keep its data and process that data
...
\item {\em On-chain\/} applications (cf.~\ptref{sp:pure.blockchain}):
  All data and processing are in the ION Blockchain.
\item {\em Off-chain\/} applications (cf.~\ptref{sp:pure.net.serv}):
  All data and processing are outside the ION Blockchain, on servers
  available through the ION Network.
\item {\em Mixed\/} applications (cf.~\ptref{sp:mixed.serv}): Some,
  but not all, data and processing are in the ION Blockchain; the rest
  are on off-chain servers available through the ION Network.
\end{itemize}
...
(commonly abbreviated as ``dapp'') completely in the ION Blockchain,
...
and all interaction with the project will be done by means of (ION
...
``ion-sites'' and ``ion-services''.)  Another extreme option is to
...
the ION Network, almost without using the ION Blockchain.
...
still be kept in the ION Blockchain (i.e., an immutable public
ledger), and any misbehavior of the service or of its users could be
punished by smart contracts.

\nxsubpoint\label{sp:ex.ion.storage} \embt(Example: keeping files
off-chain; ION Storage.)  An example of such a service is given by
{\em ION Storage}. In its simplest form, it allows users to store
...
might essentially create a ``torrent'' for a new file, and use ION DHT
...
these photographs for prolonged periods. The ION Storage technology,
...
upload files for off-chain storage. In this way a service like {\em
  ION Storage\/} becomes truly decentralized, because it does not need
...
\nxsubpoint\label{sp:ex.ion.proxy} \embt(Example: ION Proxy is a fog
service.)  {\em ION Proxy\/} provides yet another example of a fog
...
payment channels provided by {\em ION Payments\/} for processing
micropayments for the services of those proxies, with payments
collected, for instance, for every 128~KiB transferred.

\nxsubpoint \embt(Example: ION Payments is a fog service.)  The ION
...
\nxsubpoint \embt(Example: connecting to ION Payments.)  For example,
if one wants to use ION Payments (cf.~\ptref{sect:payments}), the
...
\nxsubpoint \embt(Example: uploading a file into ION Storage.)
Similarly, if one wants to upload a file into the ION Storage, she
...
option is to create a dedicated workchain inside the ION Blockchain,
...
\mysubsection{Accessing ION Services}

We have discussed in~\ptref{sect:ion.service.impl} the different
approaches one might employ for creating new services and applications
residing in the ION ecosystem. Now we discuss how these services might
be accessed, and some of the ``helper services'' that will be provided
by ION, including {\em ION DNS\/} and {\em ION Storage}.

\nxsubpoint\label{sp:ion.dns} \embt(ION DNS: a mostly on-chain
hierarchical domain name service.)  The {\em ION DNS\/} is a
...
network nodes and ION Blockchain accounts and smart contracts.

While anybody might in principle implement such a service using the
ION Blockchain, it is useful to have such a predefined service with a
...
\nxsubpoint \embt(ION DNS use cases.)  For example, a user looking to
transfer some cryptocurrency to another user or to a merchant may
prefer to remember a ION DNS domain name of the account of that user
...
Similarly, ION DNS may be used to locate account identifiers of smart
contracts or entry points of ion-services and ion-sites
(cf.~\ptref{sp:pure.net.serv}), enabling a specialized client
(``ion-browser''), or a usual internet browser combined with a
specialized ion-proxy extension or stand-alone application, to deliver
a WWW-like browsing experience to the user.

\nxsubpoint \embt(ION DNS smart contracts.)  The ION DNS is
...
of the ION DNS system will be kept, is located in the masterchain. Its
account identifier must be hardcoded into all software that wishes to
access the ION DNS database directly.
...
\nxsubpoint \embt(Values of the DNS hashmap, or ION DNS records.)  As
to the values, they are ``ION DNS records'' described by a TL-scheme
...
\nxsubpoint \embt(Translating a ION DNS domain.)  Once any full node,
acting by itself or on behalf of some light client, can look up
entries in the database of any DNS smart contract, arbitrary ION DNS
...
\nxsubpoint \embt(Translating ION DNS domains for light nodes.)  In
...
light node to verify that the answer is correct, so such ION DNS
...
shardchains, actual ION DNS domain translation would involve a
combination of these two strategies.

\nxsubpoint \embt(Dedicated ``ION DNS servers''.)  One might provide a
simple ``ION DNS server'', which would receive RPC ``DNS'' queries
...
considered part of the ION DNS service, would effectively transform it
from a distributed on-chain service into a distributed mixed service
(i.e., a ``fog service'').

This concludes our brief overview of the ION DNS service, a scalable
on-chain registry for human-readable domain names of ION Blockchain
and ION Network entities.
...
the ION VM, initialize its persistent storage from the global state of
...
\nxsubpoint\label{sp:ui.ion.serv} \embt(User interface of a
``ion-service''.)  It turns out that ``ion-services'' (i.e., services
residing in the ION Network and accepting queries through the ADNL and
...
``ion-browser'', might prompt the user to select one of the methods
...
an RPC query to the abstract address of the ``ion-service'' in
question, and the response to this query is parsed and displayed
according to the formal interface (i.e., a TL-scheme).

\nxsubpoint\label{sp:ui.ion.dns} \embt(Locating user interfaces via
ION DNS.)  The ION DNS record containing an abstract address of a
ion-service or a smart-contract account identifier might also contain
...
(be it a wallet, a ion-browser or a ion-proxy) will be able to
download the interface and interact with the entity in question (be it
a smart contract or a ion-service) in a uniform way.
...
into the address line of her ion-browser or wallet, and the rest is
handled seamlessly by the client application.

\nxsubpoint\label{sp:telegram.integr} \embt(A light wallet and ION
entity explorer can be built into Telegram Messenger clients.)  An
interesting opportunity arises at this point. A light wallet and ION
...
be able to send hyperlinks to ION entities and resources by including
ION URIs (cf.~\ptref{sp:ion.uri}) in messages; such hyperlinks, if
...
\nxsubpoint \embt(``ion-sites'' as ion-services supporting an HTTP
interface.)  A {\em ion-site\/} is simply a ion-service that supports
an HTTP interface, perhaps along with some other interfaces. This
support may be announced in the corresponding ION DNS record.

\nxsubpoint \embt(Hyperlinks.)  Notice that the HTML pages returned by
ion-sites may contain {\em ion-hyperlinks}---that is, references to
other ion-sites, smart contracts and accounts by means of specially
crafted URI schemes (cf.~\ptref{sp:ion.uri})---containing either
abstract network addresses, account identifiers, or human-readable ION
DNS domains. Then a ``ion-browser'' might follow such a hyperlink when
the user selects it, detect the interface to be used, and display a
user interface form as outlined in \ptref{sp:ui.smartc}
and~\ptref{sp:ui.ion.serv}.

\nxsubpoint\label{sp:ion.uri} \embt(Hyperlink URLs may specify some
parameters.)  The hyperlink URLs may contain not only a (ION) DNS
...
\begin{quote}
\texttt{ion://}\textit{<domain>}\texttt{/}\textit{<method>}\texttt{?}%
\textit{<field1>}\texttt{=}\textit{<value1>}\texttt{\&}%
\textit{<field2>}\texttt{=}\dots
\end{quote}
When the user selects such a link in a ion-browser, either the action
...
\nxsubpoint \embt(POST actions.)  A ion-site may embed into the HTML
pages it returns some usual-looking POST forms, with POST actions
referring either to ion-sites, ion-services or smart contracts by
means of suitable (ION) URLs. In that case, once the user fills and
submits that custom form, the corresponding action is taken, either
immediately or after an explicit confirmation.

\nxsubpoint\label{sp:ion.www} \embt(ION WWW.)  All of the above will
lead to the creation of a whole web of cross-referencing entities,
residing in the ION Network, which would be accessible to the end user
through a ion-browser, providing the user with a WWW-like browsing
...
\nxsubpoint \embt(Advantages of ION WWW.)  This ``ION WWW'' of
...
network anonymity can be easily preserved by means of ION Proxy, and
all services will be effectively unblockable. Micropayments are also
possible and easy, because ion-browsers can be integrated with the ION
Payments system.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\clearpage
\mysection{ION Payments}\label{sect:payments}

The last component of the ION Project we will briefly discuss in this
text is {\em ION Payments}, the platform for (micro)payment channels
...
use them for micropayments. For example, a ION file-storing service
might charge the user for every 128 KiB of downloaded data, or a paid
ION Proxy might require some tiny micropayment for every 128 KiB of
traffic relayed.
...
\nxsubpoint\label{sp:ion.smart.pc.supp} \embt(ION VM support for
``smart'' payment channels.)  The ION VM, used to run the code of ION
...
Now we are ready to discuss the ``lightning network'' of ION Payments
...
how they can be implemented in ION.
...
and network technologies of the ION project are adequate to the task
of creating {\em ION Payments}, a platform for off-chain instant money
transfers and micropayments. This platform can be extremely useful
for services residing in the ION ecosystem, allowing them to easily
...
To achieve the necessary scalability, we proposed the {\em ION
...
(cf.~\ptref{sp:instant.hypercube}). A brief comparison of the ION
...
$\$0.1$ (USD). Every subsequent Gram to be sold (by the ION Reserve,
controlled by the ION Foundation) will be priced one billionth higher
...
ION Reserve; they would choose to buy their Grams on the free market
...
obtain new Grams from the ION Reserve. This means that the market
...
ION Reserve), the ION Reserve reserves the right to buy some Grams
back and decrease $n$, the total quantity of Grams in
circulation. This may be required to prevent sudden falls of the
Gram exchange rate.

\nxsubpoint \embt(Selling new Grams at a higher price.)  The ION
...
\nxpoint\label{sp:unalloc.gr} \embt(Using unallocated Grams.)  The ION
...
the ION Reserve and some other accounts explicitly linked to it---only
as validator stakes (because the ION Foundation itself will likely
have to provide most of the validators during the first deployment
phase of the ION Blockchain), and for voting in the masterchain for or
...
ION Foundation (i.e., its creators---the development team). This also
means that the ION Foundation will have a majority of votes during the
first deployment phase of the ION Blockchain, which may be useful if a
...
under control of the ION Foundation, the system will become more
...
supply) will be transferred during the deployment of the ION
Blockchain to a special account controlled by the ION Foundation, and
then some ``rewards'' may be paid from this account to the developers
of the open source ION software, with a minimum two-year vesting
period.

\nxsubpoint\label{sp:ION.own.grams} \embt(The ION Foundation needs
Grams for operational purposes.)  Recall that the ION Foundation will
receive the fiat and cryptocurrency obtained by selling Grams from the
ION Reserve, and will use them for the development and deployment of
the ION Project. For instance, the original set of validators, as well
as an initial set of ION Storage and ION Proxy nodes may be installed
by the ION Foundation.
