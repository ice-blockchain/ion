...
int main(int argc, char *argv[]) {
  ...
  for (td::uint32 start = att; att < start + 5; att++) {
    ...
    auto overlay_id_full =
        ion::create_serialize_tl_object<ion::ton_api::pub_overlay>(td::BufferSlice(PSTRING() << "TEST" << att));
    ...
    scheduler.run_in_context([&] {
      /*td::actor::send_closure(overlay_manager, &ion::overlay::Overlays::get_stats,
                              [&](td::Result<ion::tl_object_ptr<ion::ton_api::engine_validator_overlaysStats>> R) {
                                if (R.is_ok()) {
