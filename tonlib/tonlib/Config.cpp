...
td::Result<Config> Config::parse(std::string str) {
  ...
  Config res;
  ion::ton_api::liteclient_config_global conf;
  TRY_STATUS(ion::ton_api::from_json(conf, json.get_object()));
  ...
  res.zero_state_id = ion::create_block_id(conf.validator_->zero_state_);
  if (conf.validator_->init_block_) {
    res.init_block_id = ion::create_block_id(conf.validator_->init_block_);
  }

  for (auto &fork : conf.validator_->hardforks_) {
    res.hardforks.push_back(ion::create_block_id(fork));
  }
