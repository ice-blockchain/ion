...
class AdnlNode : public td::actor::Actor {
 private:
  ...
  void receive_query(td::UInt256 src, td::UInt256 dst, td::uint64 query_id, td::BufferSlice data) {
    ...
    td::actor::send_closure(peer_table_, &ion::AdnlPeerTable::answer_query, dst, src, query_id,
                            ion::create_tl_object<ion::ton_api::testObject>());
  }
 ...
 public:
  ...
  void add_local_id(ion::tl_object_ptr<ion::ton_api::adnl_id_Pk> pk_) {
    auto pub_ = ion::get_public_key(pk_);
    local_id_ = ion::adnl_short_id(pub_);
    std::cout << "local_id = '" << local_id_ << "'\n";
    auto x = ion::create_tl_object<ion::ton_api::adnl_address_udp>(ip_, port_);
    auto v = std::vector<ion::tl_object_ptr<ion::ton_api::adnl_Address>>();
    v.push_back(ion::move_tl_object_as<ion::ton_api::adnl_Address>(x));
    auto y =
        ion::create_tl_object<ion::ton_api::adnl_addressList>(std::move(v), static_cast<td::int32>(td::Time::now()));

    LOG(INFO) << "local_addr_list: " << ion::ton_api::to_string(y);
  ...
  void add_foreign(ion::tl_object_ptr<ion::ton_api::adnl_id_Full> id,
                   ion::tl_object_ptr<ion::ton_api::adnl_addressList> addr_list) {
    ...
    if (local_id_set_) {
      for (auto it = ping_ids_.begin(); it != ping_ids_.end(); it++) {
        ...
        td::actor::send_closure(peer_table_, &ion::AdnlPeerTable::send_query, local_id_, *it, std::move(P),
                                td::Timestamp::in(5), ion::create_tl_object<ion::ton_api::getTestObject>());
      }
    }
...
int main(int argc, char *argv[]) {
  ...
  p.add_option('i', "id", "sets local id", [&](td::Slice id) {
    td::actor::send_closure(x, &AdnlNode::add_local_id,
                            ion::create_tl_object<ion::ton_api::adnl_id_pk_unenc>(id.str()));
    return td::Status::OK();
  });
  p.add_option('P', "peer", "adds peer id@host:port", [&](td::Slice id) {
    ...
    auto f_id = ion::create_tl_object<ion::ton_api::adnl_id_unenc>(s1.str());
    td::IPAddress addr;
    ...
    auto f_addr = ion::create_tl_object<ion::ton_api::adnl_address_udp>(addr.get_ipv4(), addr.get_port());
    std::vector<ion::tl_object_ptr<ion::ton_api::adnl_Address>> vv;
    vv.push_back(ion::move_tl_object_as<ion::ton_api::adnl_Address>(f_addr));

    auto f_addr_list =
        ion::create_tl_object<ion::ton_api::adnl_addressList>(std::move(vv), static_cast<int>(td::Time::now()));

    td::actor::send_closure(x, &AdnlNode::add_foreign, ion::move_tl_object_as<ion::ton_api::adnl_id_Full>(f_id),
