...
template <typename T>
inline void db_get(DbType& db, td::Bits256 key, bool allow_not_found, td::Promise<ion::tl_object_ptr<T>> promise) {
  db.get(key, [allow_not_found, promise = std::move(promise)](td::Result<DbType::GetResult> R) mutable {
    ...
    promise.set_result(ion::fetch_tl_object<T>(r.value, true));
  });
}

template <typename T>
inline td::Result<ion::tl_object_ptr<T>> db_get(td::KeyValue& db, td::Bits256 key, bool allow_not_found) {
  std::string value;
  ...
  return ion::fetch_tl_object<T>(td::Slice(value), true);
}
