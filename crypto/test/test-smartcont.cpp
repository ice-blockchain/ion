...
TEST(Tonlib, WalletV3) {
  ...
  CHECK(get_wallet_v3_source()->get_hash() == ion::WalletV3::get_init_code(2)->get_hash());
  ...
  td::Ref<vm::Cell> ext_init_message = ion::GenericAccount::create_ext_message(
      address, ion::GenericAccount::get_init_state(wallet->get_state()), init_message);
  LOG(ERROR) << "-------";
  ...
  CHECK(priv_key.get_public_key().ok().as_octet_string() ==
        ion::GenericAccount::get_public_key(*wallet).ok().as_octet_string());

  auto gift_message = ion::GenericAccount::create_ext_message(
...
TEST(Tonlib, HighloadWallet) {
  ...
  td::Ed25519::PrivateKey priv_key{td::SecureString{new_wallet_pk}};
  auto pub_key = priv_key.get_public_key().move_as_ok();
  ion::HighloadWallet::InitData init_data(pub_key.as_octet_string(), 239);

  auto wallet = ion::HighloadWallet::create(init_data, -1);
  auto address = wallet->get_address();
  CHECK(address.addr.as_slice() == td::Slice(new_wallet_addr).substr(0, 32));
  ASSERT_EQ(239u, wallet->get_wallet_id().ok());
  ASSERT_EQ(0u, wallet->get_seqno().ok());
  CHECK(pub_key.as_octet_string() == wallet->get_public_key().ok().as_octet_string());
  CHECK(pub_key.as_octet_string() == ion::GenericAccount::get_public_key(*wallet).ok().as_octet_string());

  CHECK(address.addr.as_slice() == td::Slice(new_wallet_addr).substr(0, 32));

  auto init_message = wallet->get_init_message(priv_key).move_as_ok();
  td::Ref<vm::Cell> res = ion::GenericAccount::create_ext_message(
      address, ion::GenericAccount::get_init_state(wallet->get_state()), init_message);
  ...
  std::string order;
  std::vector<ion::HighloadWallet::Gift> gifts;
  ...
  init_data.seqno = 123;
  wallet = ion::HighloadWallet::create(init_data, -1);
  fift_output.source_lookup.set_os_time(std::make_unique<ZeroOsTime>());
...
TEST(Tonlib, HighloadWalletV2) {
  ...
  td::Ed25519::PrivateKey priv_key{td::SecureString{new_wallet_pk}};
  auto pub_key = priv_key.get_public_key().move_as_ok();
  ion::HighloadWalletV2::InitData init_data(pub_key.as_octet_string(), 239);

  auto wallet = ion::HighloadWalletV2::create(init_data, -1);
  auto address = wallet->get_address();
  ...
  std::string order;
  std::vector<ion::HighloadWalletV2::Gift> gifts;
  auto add_order = [&](td::Slice dest_str, td::int64 gramms) {
    auto g = td::to_string(gramms);
    if (g.size() < 10) {
      g = std::string(10 - g.size(), '0') + g;
...
TEST(Tonlib, RestrictedWallet) {
  ...
  //auto wallet = ion::RestrictedWallet::create(init_data, -1);
  ...
  //auto res = ion::GenericAccount::create_ext_message(address, wallet->get_init_state(),
...
void check_wallet_seqno(td::Ref<ion::HighloadWalletV2> wallet, td::uint32 seqno) {
}
void check_wallet_seqno(td::Ref<ion::WalletInterface> wallet, td::uint32 seqno) {
}
template <class T>
...
struct CreatedWallet {
  ...
  td::Ref<ion::WalletInterface> wallet;
};
...
class InitWallet {
 public:
  CreatedWallet operator()(int revision) const {
    ion::WalletInterface::DefaultInitData init_data;
    auto priv_key = td::Ed25519::generate_private_key().move_as_ok();
    auto pub_key = priv_key.get_public_key().move_as_ok();
...
template <>
CreatedWallet InitWallet<ion::RestrictedWallet>::operator()(int revision) const {
  auto init_priv_key = td::Ed25519::generate_private_key().move_as_ok();
  auto init_pub_key = init_priv_key.get_public_key().move_as_ok();
  auto priv_key = td::Ed25519::generate_private_key().move_as_ok();
  auto pub_key = priv_key.get_public_key().move_as_ok();
...
void do_test_wallet(int revision) {
...

  {
    ...
    CHECK(!cwallet.write()
               .send_external_message(send_gifts, ion::SmartContract::Args().set_now(valid_until + 1))
               .success);
  }
  //TODO: make wallet work (or not) with now == valid_until
  auto ans = iwallet.write().send_external_message(send_gifts, ion::SmartContract::Args().set_now(valid_until - 1));
  CHECK(ans.success);
...
TEST(Tonlib, Wallet) {
  do_test_wallet<ion::WalletV3>();
  do_test_wallet<ion::WalletV4>();
  do_test_wallet<ion::HighloadWallet>();
  do_test_wallet<ion::HighloadWalletV2>();
  do_test_wallet<ion::RestrictedWallet>();
}

namespace std {  // ouch
bool operator<(const ion::MultisigWallet::Mask& a, const ion::MultisigWallet::Mask& b) {
...
TEST(Smartcon, Multisig) {
  auto ms_lib = ion::MultisigWallet::create();
  ...
  auto ms = ion::MultisigWallet::create(init_state);

  td::uint32 now = 0;
  auto args = [&now]() -> ion::SmartContract::Args { return ion::SmartContract::Args().set_now(now); };
...

  {
    ...
    ion::MultisigWallet::QueryBuilder qb(wallet_id, query_id, vm::CellBuilder().finalize());
...
TEST(Smartcont, MultisigStress) {
  ...
  auto verify = [&] {
    ...
    std::set<std::tuple<td::uint64, ion::MultisigWallet::Mask, std::string>> s;
    std::set<std::tuple<td::uint64, ion::MultisigWallet::Mask, std::string>> t;
  ...
  auto sign_query = [&](Query& query, Mask mask) {
    auto qb = ion::MultisigWallet::QueryBuilder(wallet_id, query.id, query.message);
    int first_i = -1;
  ...
  auto rand_sign = [&] {
    ...
    ASSERT_EQ(expect_is_ready, (state.state == ion::MultisigWallet::QueryState::Sent));
...
class MapDns {
 public:
  ...
  struct Entry {
    ...
    bool operator==(const ion::DnsInterface::Entry& other) const {
  ...
  using CombinedActions = ion::ManualDns::CombinedActions<Action>;
  void update_combined(td::Span<Action> actions) {
    LOG(ERROR) << "BEGIN";
    LOG(ERROR) << td::format::as_array(actions);
    auto combined_actions = ion::ManualDns::combine_actions(actions);
  ...
  std::vector<Entry> resolve(td::Slice name, td::Bits256 category) {
    ...
    if (name.empty()) {
      ...
      while (it == entries_.end()) {
        auto sz = name.find('.');
        category = ion::DNS_NEXT_RESOLVER_CATEGORY;
        if (sz != td::Slice::npos) {
  ...
  void update(td::Span<Action> entries) {
    if (dns_.not_null()) {
      auto smc_actions = td::transform(entries, [](auto& entry) {
        ion::DnsInterface::Action action;
...
TEST(Smartcont, DnsManual) {
  ...
  test_entry_data(ManualDns::EntryData::adnl_address(ion::Bits256{}));
  ...
  {
    auto actions_ext =
        ion::ManualDns::parse("delete.name one\nset one 1 TEXT:one\ndelete.name two\nset two 2 TEXT:two").move_as_ok();

    auto actions = td::transform(actions_ext, [](auto& action) {
      td::optional<std::string> data;
      if (action.data) {
        data = action.data.value().data.template get<ion::ManualDns::EntryDataText>().text;
      }
...
TEST(Smarcont, Channel) {
  auto code = ion::SmartContractCode::get_code(ion::SmartContractCode::PaymentChannel);
  Config config;
  ...
  auto channel = ion::SmartContract::create(ion::SmartContract::State{code, data_cell});
...
#define expect_ok(description, e) expect_code(description, 0, e)
  ...
  expect_code("Trying to invoke a timeout while channel is empty", no_timeout,
              channel.write().send_external_message(MsgTimeoutBuilder().finalize(),
                                                    ion::SmartContract::Args().set_now(1000000)));
  ...
  expect_code("Internal init message with not enough value", msg_value_too_small,
              channel.write().send_internal_message(
                  ...
                  ion::SmartContract::Args().set_amount(100)));
  expect_code(
      ...
      channel.write().send_internal_message(MsgInitBuilder().inc_A(1000).min_B(2000).with_a_key(&a_pkey).finalize(),
                                            ion::SmartContract::Args().set_amount(1000)));
  ...
  expect_code(
      ...
      channel.write().send_external_message(MsgTimeoutBuilder().finalize(), ion::SmartContract::Args().set_now(0)));

  {
    auto channel_copy = channel;
    expect_ok("Invoke a timeout", channel_copy.write().send_external_message(MsgTimeoutBuilder().finalize(),
                                                                             ion::SmartContract::Args().set_now(21)));
  ...
  }
  {
    ...
    expect_ok("B init with inc_B < min_B. Leads to immediate payout",
              channel_copy.write().send_internal_message(
                  ...
                  ion::SmartContract::Args().set_amount(1500)));
  ...
  expect_ok("B init with (inc_B = 2000, min_A = 1, min_A = 1000)",
            channel.write().send_internal_message(
                ...
                ion::SmartContract::Args().set_amount(2000)));
