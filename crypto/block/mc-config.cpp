...
td::Result<std::unique_ptr<ConfigInfo>> ConfigInfo::extract_config(Ref<vm::Cell> mc_state_root,
                                                                   ion::BlockIdExt mc_block_id, int mode) {
...
td::optional<ion::NewConsensusConfig> Config::get_new_consensus_config(ion::WorkchainId wc) const {
  ...
  auto c2 = (wc == ion::masterchainId ? rec.mc : rec.shard)->prefetch_ref();
  if (c2.is_null()) {
    return {};
  }
  ...
  if (gen::unpack_cell(c2, r1)) {
    return ion::NewConsensusConfig{.target_rate_ms = r1.target_rate_ms,
                                   ...
                                   .consensus = ion::NewConsensusConfig::NullConsensus{}};
  }
  gen::NewConsensusConfig::Record_simplex_config r2;
  if (gen::unpack_cell(c2, r2)) {
    return ion::NewConsensusConfig{
        .target_rate_ms = r2.target_rate_ms,
        .max_block_size = consensus_config.max_block_size,
        .max_collated_data_size = consensus_config.max_collated_data_size,
        .use_quic = r2.use_quic,
        .consensus = ion::NewConsensusConfig::Simplex{.slots_per_leader_window = r2.slots_per_leader_window,
...
std::vector<ion::ValidatorDescr> Config::compute_validator_set(ion::ShardIdFull shard,
                                                               const block::TotalValidatorSet& vset, ion::UnixTime time,
...
std::vector<ion::ValidatorDescr> TotalValidatorSet::export_validator_set() const {
...
std::map<ion::Bits256, int> TotalValidatorSet::compute_validator_map() const {
...
std::vector<ion::ValidatorDescr> Config::do_compute_validator_set(const CatchainValidatorsConfig& ccv_conf,
                                                                  ion::ShardIdFull shard, const TotalValidatorSet& vset,
