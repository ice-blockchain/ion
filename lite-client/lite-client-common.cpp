...
td::Result<std::unique_ptr<block::BlockProofChain>> deserialize_proof_chain(
  ...
  auto chain = std::make_unique<block::BlockProofChain>(ion::create_block_id(f->from_), ion::create_block_id(f->to_));
  chain->complete = f->complete_;
  for (auto& s : f->steps_) {
    bool ok = false;
    td::BufferSlice dest_proof, proof, state_proof;
    ion::lite_api::downcast_call(*s, td::overloaded(
                                         [&](ion::lite_api::liteServer_blockLinkBack& s) {
                                           auto& link = chain->new_link(ion::create_block_id(s.from_),
                                                                        ion::create_block_id(s.to_), s.to_key_block_);
                                           link.is_fwd = false;
                                         ...
                                         [&](ion::lite_api::liteServer_blockLinkForward& s) {
                                           auto& link = chain->new_link(ion::create_block_id(s.from_),
                                                                        ion::create_block_id(s.to_), s.to_key_block_);
                                           link.is_fwd = true;
