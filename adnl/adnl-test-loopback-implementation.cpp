...
AdnlAddressList TestLoopbackNetworkManager::generate_dummy_addr_list(bool empty) {
  auto obj = ion::create_tl_object<ion::ton_api::adnl_address_udp>(1, 1);
  auto objv = std::vector<ion::tl_object_ptr<ion::ton_api::adnl_Address>>();
  objv.push_back(std::move(obj));
  td::uint32 now = Adnl::adnl_start_time();
  auto addrR = ion::adnl::AdnlAddressList::create(
      ion::create_tl_object<ion::ton_api::adnl_addressList>(std::move(objv), empty ? 0 : now, empty ? 0 : now, 0, 0));
  addrR.ensure();
