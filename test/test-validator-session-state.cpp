...
class Description : public ion::validatorsession::ValidatorSessionDescription {
 public:
  ...
  td::Result<td::uint32> get_source_idx_safe(ion::PublicKeyHash id) const override {
  ...
  ion::validatorsession::ValidatorSessionCandidateId candidate_id(
    ...
    auto obj = ion::create_tl_object<ion::ton_api::validatorSession_candidateId>(get_source_id(src_idx).tl(), root_hash,
...
int main() {
...

  {
    ...
    for (td::uint32 i = 0; i < total_nodes; i++) {
      ...
      CHECK(act);
      CHECK(act->get_id() == ion::ton_api::validatorSession_message_empty::ID);
    }

    {
      auto act = ion::create_tl_object<ion::ton_api::validatorSession_message_submittedBlock>(
    ...
    for (td::uint32 i = 0; i < total_nodes; i++) {
      auto act = s->create_action(desc, i, att);
      CHECK(act);
      LOG_CHECK(act->get_id() == ion::ton_api::validatorSession_message_empty::ID) << act.get();
    }
    ...
    for (td::uint32 i = 0; i < 2 * total_nodes / 3; i++) {
      ...
      sig.as_slice()[0] = 127;
      auto act = ion::create_tl_object<ion::ton_api::validatorSession_message_approvedBlock>(0, c2, std::move(sig));
    ...
    for (td::uint32 i = 0; i < total_nodes; i++) {
      auto act = s->create_action(desc, i, att);
      CHECK(act);
      LOG_CHECK(act->get_id() == ion::ton_api::validatorSession_message_vote::ID) << act.get();
      ...
      if (i < 2 * total_nodes / 3) {
        LOG_CHECK(act2->get_id() == ion::ton_api::validatorSession_message_empty::ID) << "i=" << i << " " << act2.get();
      } else {
        LOG_CHECK(act2->get_id() == ion::ton_api::validatorSession_message_precommit::ID)
            << "i=" << i << " " << act2.get();
      }
    }
    for (td::uint32 j = 1; j < opts.max_round_attempts; j++) {
      ...
      CHECK(act);
      LOG_CHECK(act->get_id() == ion::ton_api::validatorSession_message_vote::ID) << "j=" << j << " " << act.get();
    }
    for (td::uint32 j = opts.max_round_attempts; j < opts.max_round_attempts + 10; j++) {
      auto act = s->create_action(desc, 0, att + j);
      CHECK(act);
      LOG_CHECK(act->get_id() == ion::ton_api::validatorSession_message_empty::ID) << "j=" << j << " " << act.get();
    }
    auto s_copy = s;
    auto att_copy = att;
    for (td::uint32 i = 0; i < total_nodes; i++) {
      ...
      if (i <= 2 * total_nodes / 3) {
        LOG_CHECK(act->get_id() == ion::ton_api::validatorSession_message_precommit::ID)
            << "i=" << i << " " << act.get();
      } else {
        LOG_CHECK(act->get_id() == ion::ton_api::validatorSession_message_empty::ID) << "i=" << i << " " << act.get();
      }
    ...
    for (td::uint32 i = 0; i < 2 * total_nodes / 3; i++) {
      td::BufferSlice sig{1};
      sig.as_slice()[0] = 126;
      auto act = ion::create_tl_object<ion::ton_api::validatorSession_message_commit>(0, c2, std::move(sig));
      s = ion::validatorsession::ValidatorSessionState::action(desc, s, i, att, act.get());
      CHECK(s);
      s = ion::validatorsession::ValidatorSessionState::move_to_persistent(desc, s);
      CHECK(s);
    ...
    for (td::uint32 i = 0; i < total_nodes / 3; i++) {
      ...
      LOG_CHECK(act->get_id() == ion::ton_api::validatorSession_message_precommit::ID) << "i=" << i << " " << act.get();
    ...
    do {
      ...
      for (td::uint32 i = 0; i < total_nodes; i++) {
        ...
        if (i < total_nodes / 3) {
          LOG_CHECK(act->get_id() == ion::ton_api::validatorSession_message_vote::ID) << "i=" << i << " " << act.get();
        } else {
          LOG_CHECK(act->get_id() == ion::ton_api::validatorSession_message_empty::ID) << "i=" << i << " " << act.get();
        }
    ...
    for (td::uint32 i = 0; i < total_nodes; i++) {
      td::BufferSlice sig{1};
      sig.as_slice()[0] = 127;
      auto act = ion::create_tl_object<ion::ton_api::validatorSession_message_approvedBlock>(0, c1, std::move(sig));
      s = ion::validatorsession::ValidatorSessionState::action(desc, s, i, att, act.get());
      CHECK(s);
      s = ion::validatorsession::ValidatorSessionState::move_to_persistent(desc, s);
      CHECK(s);
  ...
  for (td::uint32 ver = 0; ver < 2; ver++) {
    ...
    for (td::uint32 ri = 0; ri < 100000; ri++) {
      ...
      if (vec.size() > 0 && myrand() <= approve_prob) {
        ...
        auto act =
            ion::create_tl_object<ion::ton_api::validatorSession_message_approvedBlock>(round, id, std::move(sig));
        s = ion::validatorsession::ValidatorSessionState::action(desc, s, x, att, act.get());
        CHECK(s);
      }
      ...
      if (found && myrand() <= sign_prob) {
        ...
        auto act = ion::create_tl_object<ion::ton_api::validatorSession_message_commit>(round, id, std::move(sig));
        s = ion::validatorsession::ValidatorSessionState::action(desc, s, x, att, act.get());
        CHECK(s);
      }
      ...
      while (true) {
        ...
        bool stop = false;
        if (act->get_id() == ion::ton_api::validatorSession_message_empty::ID) {
          stop = true;
        }
        s = ion::validatorsession::ValidatorSessionState::action(desc, s, x, att, act.get());
        CHECK(s);
