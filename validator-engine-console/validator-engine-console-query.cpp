...
td::Status GetCollatorOptionsJsonQuery::send() {
  auto b =
      ion::create_serialize_tl_object<ion::ton_api::engine_validator_getCollatorOptionsJson>();
  td::actor::send_closure(console_, &ValidatorEngineConsole::envelope_send_query, std::move(b), create_promise());
  return td::Status::OK();
}
...
td::Status GetAdnlStatsJsonQuery::send() {
  auto b =
      ion::create_serialize_tl_object<ion::ton_api::engine_validator_getAdnlStats>(all_);
  td::actor::send_closure(console_, &ValidatorEngineConsole::envelope_send_query, std::move(b), create_promise());
  return td::Status::OK();
}
    ...
    if (!local_id->current_decrypt_.empty()) {
      std::sort(local_id->current_decrypt_.begin(), local_id->current_decrypt_.end(),
                [](const ion::tl_object_ptr<ion::ton_api::adnl_stats_ipPackets> &a,
                   const ion::tl_object_ptr<ion::ton_api::adnl_stats_ipPackets> &b) {
