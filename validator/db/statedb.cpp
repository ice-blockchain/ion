...
void StateDb::update_hardforks(std::vector<BlockIdExt> blocks, td::Promise<td::Unit> promise) {
  ...
  std::vector<tl_object_ptr<ion_api::tonNode_blockIdExt>> vec;
...
void StateDb::add_persistent_state_description(td::Ref<PersistentStateDescription> desc,
                                               td::Promise<td::Unit> promise) {
  std::string value;
  auto list_key = create_hash_tl_object<ion_api::db_state_key_persistentStateDescriptionsList>();
  auto R = kv_->get(list_key.as_slice(), value);
  R.ensure();
  tl_object_ptr<ion_api::db_state_persistentStateDescriptionsList> list;
  if (R.ok() == td::KeyValue::GetStatus::Ok) {
    auto F = fetch_tl_object<ion_api::db_state_persistentStateDescriptionsList>(value, true);
    F.ensure();
    list = F.move_as_ok();
  } else {
    list = create_tl_object<ion_api::db_state_persistentStateDescriptionsList>(
        std::vector<tl_object_ptr<ion_api::db_state_persistentStateDescriptionHeader>>());
  }
  ...
  for (auto& obj : list->list_) {
    ...
    if (end_time <= now) {
      auto key =
          create_hash_tl_object<ion_api::db_state_key_persistentStateDescriptionShards>(obj->masterchain_id_->seqno_);
  ...
  std::vector<tl_object_ptr<ion_api::tonNode_blockIdExt>> shard_blocks;
  for (const BlockIdExt& block_id : desc->shard_blocks) {
    shard_blocks.push_back(create_tl_block_id(block_id));
  }
  auto key =
      create_hash_tl_object<ion_api::db_state_key_persistentStateDescriptionShards>(desc->masterchain_id.seqno());
  kv_->set(key.as_slice(),
           create_serialize_tl_object<ion_api::db_state_persistentStateDescriptionShards>(std::move(shard_blocks))
               .as_slice())
      .ensure();

  list->list_.push_back(create_tl_object<ion_api::db_state_persistentStateDescriptionHeader>(
...
void StateDb::get_persistent_state_descriptions(td::Promise<std::vector<td::Ref<PersistentStateDescription>>> promise) {
  std::string value;
  auto R = kv_->get(create_hash_tl_object<ion_api::db_state_key_persistentStateDescriptionsList>().as_slice(), value);
  R.ensure();
  ...
  for (const auto& obj : F.ok()->list_) {
    ...
    auto F2 = fetch_tl_object<ion_api::db_state_persistentStateDescriptionShards>(value, true);
    F2.ensure();
